...
<div class="container mt-5">
  ...
  <form action="{{ route('addressinfo.store') }}" method="POST">
    ...
    <div class="form-group">
        ...
        <input type="text" name="ward_vdc" class="form-control" value="{{ old('ward_vdc') }}" required>
    </div>
    <div class="form-group">
        <label for="tole_street">Tole/Street</label>
        <input type="text" name="tole_street" class="form-control" value="{{ old('tole_street') }}" required>
    </div>
    <div class="form-group">
        <label for="is_permanent">Is Permanent</label>
        <select name="is_permanent" class="form-control" required>
            <option value="">-- Select --</option>
            <option value="1" {{ old('is_permanent') == '1' ? 'selected' : '' }}>Yes</option>
            <option value="0" {{ old('is_permanent') == '0' ? 'selected' : '' }}>No</option>
        </select>
    </div>
