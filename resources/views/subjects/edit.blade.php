    ...
    <form action="{{ route('subjects.update', $subject->id) }}" method="POST">
        ...
        <div class="mb-3">
            ...
            <input type="text" name="class" class="form-control" id="class" 
                value="{{ old('class', $subject->class) }}" required>
        </div>

        <div class="mb-3">
            <label for="faculty_id" class="form-label">Faculty ID</label>
            <input type="number" name="faculty_id" class="form-control" id="faculty_id" 
                value="{{ old('faculty_id', $subject->faculty_id) }}" required>
        </div>

        <div class="mb-3">
            <label for="batch_id" class="form-label">Batch ID</label>
            <input type="number" name="batch_id" class="form-control" id="batch_id" 
                value="{{ old('batch_id', $subject->batch_id) }}" required>
        </div>
